// ==========================================================================
//  tb_wallace_16x16_wrapper_regio.v
//  Drives wallace_16x16_wrapper_regio (the FULLY registered wrapper, latency 2 clocks).
//
//  Used for
...
//  on every falling edge and checks the output 2 clock(s) later against a
//  2-deep expectation pipeline. This matters for power: if the bench only
...
module tb_wallace_16x16_wrapper_regio;

    parameter real CLK_PERIOD = 20.0;      // ns
    localparam integer LAT    = 2;     // pipeline depth of the wrapper
    ...
    wallace_16x16_wrapper_regio dut (.clk(clk), .a(a), .b(b), .product(product));
    ...
    initial begin
        ...
        $display("tb_wallace_16x16_wrapper_regio: CLK_PERIOD=%0.3f ns -> %0.3f MHz, one multiply per clock",
    ...
    initial begin
        ...
        $display("tb_wallace_16x16_wrapper_regio: nrand=%0d  seed=%0d  fullsweep=%0b  vectors=%0d  latency=%0d clk",
